...
8. **双语模式 + 上游复用（1.18.0+）**：`--lang` 既接受单语（`zh` / `en` / `ja`）也接受双语组合（`zh+en` / `en+zh` / `zh+ja` / `ja+zh` / `en+ja` / `ja+en`，共 6 种排列；3 语暂不支持）。**架构关键：上游 3 stage（Scanner / Verifier / Fact-Extractor）只跑 1 次**——因为它们是英文 language-agnostic 的；Writer + Editor **per-lang fan out 但 SEQUENTIAL**（先 primary-lang 跑完再跑 secondary-lang，**禁止并行**），每语一个 subagent 实例，写 `out_md_{lang}` / `out_docx_{lang}`，pandoc 也按序跑 N 次。**为什么顺序而不并行**：(a) 避免未实测的并行 subagent 调度路径（claude-code#21318 是工具访问 bug，与并行调度正交，但并行 spawn opus subagent 在本流水线从未验证过）；(b) 避免 WebSearch/WebFetch 并发翻倍触发 API 速率限制；(c) 顺序跑能让第二个 Writer 命中 Verifier bundle + Manifest 的 prompt cache（0.1× 成本）；(d) hook 串行化更可控。代价是墙钟时间 +5 分钟左右，但 token 成本与并行完全相同。**邮件 Step 10**：双语模式下 subject 用 primary-lang + 双语 tag（`（中英双语）` / `(Bilingual EN+ZH)`）；body 堆叠 primary-lang 全文 + `━━━━━━━━━━━` 分隔线 + secondary-lang 全文；`--email-attach both` 附 4 个文件（zh.md / zh.docx / en.md / en.docx），`docx` 附 2 个，`md` 附 2 个，`none` 附 0。`send-report-email.py --attach` 已是 `nargs="*"`，**无需改 Python 脚本**。每个 lang 派生字段（`out_md_{lang}` / `country_display_{lang}` / `date_display_{lang}`）独立计算；filename ASCII-only 自检（zh/ja 必须翻译过 country）按 per-lang 验。Failure：单边 Writer 失败默认 halt + 报错。真源 = `references/language-spec.md` § Bilingual Mode + `references/email-spec.md` § Bilingual subject + § Bilingual body；顺序执行的论据真源 = `SKILL.md` Step 8 § Bilingual execution order。
