...
> 覆盖范围：新闻（T1-T4）、搜索引擎已收录且可抓取的公开 Reddit 讨论与 X 帖文。未被收录、需登录或无法抓取的社交内容不在本次覆盖内；Facebook 和 Threads 未纳入 v1。每条负面判断均附源文件逐字引用，收件人应自行核实 URL 后再行动。本报告由 `sci-research` 插件（`reputation-track`）自动生成。

`lang=en`:

> Coverage: News (T1-T4), plus publicly indexed and fetchable Reddit threads and X posts. Social content that is unindexed, login-gated, or unavailable to fetch is outside this run's coverage; Facebook and Threads are excluded in v1. Each adverse finding includes a verbatim quote from the source; recipients should verify the URL before acting. Generated by the `sci-research` plugin (`reputation-track`).
