...
> Coverage: News (T1-T4), plus publicly indexed and openable Reddit threads and X posts. Social content that is unindexed, login-gated, or unavailable to open is outside this run's coverage; Facebook and Threads are excluded in v1. Each adverse finding includes a verbatim quote from the source; recipients should verify the URL before acting. Generated by the `sci-research` plugin (`reputation-track`).
