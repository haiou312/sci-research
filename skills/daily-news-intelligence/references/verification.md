...
**Dispatch rule.** Every stage spawns as the built-in **`general-purpose`** subagent with the corresponding `skills/daily-news-intelligence/agents/<name>.md` **body embedded in the prompt** and an **explicit `model` arg**. See `skills/daily-news-intelligence/SKILL.md` § Subagent Dispatch Rule for the procedure and `CLAUDE.md` § 项目定位 point 6 for the #21318 rationale. The names below refer to the **prompt-template files** in `skills/daily-news-intelligence/agents/`.
