...
- Do not validate the `URL` value by presentation syntax or domain. A raw `news.google.com` URL and a Markdown link whose target is that URL are equally valid and must never trigger a Scanner retry or stop the pipeline.
...
| Condition | Handling |
|-----------|----------|
| One category Scanner errors or violates the minimal structural schema | Retry that category once with the same exact role and one-sentence task prompt. URL presentation style and URL domain are never schema violations. If the retry fails, STOP before Verifier and identify the category. |
