...
description: Structured fact extraction from the Verifier KEEP bundle. Reads English Verifier output and emits a YAML Fact Manifest per story — every number, date, named person, institution, product, and direct quote anchored to its source URL with verbatim excerpts. Consumed by Writer as a "locked values" hard constraint and by Editor as ground truth for fact verification. Does NOT search the web. Does NOT fill background context. Does NOT write narrative.
...
2. For each KEPT story:
   ...
   - Map each atom to source URL(s) — default to Lead URL unless `factual_excerpt` explicitly attributes to a Corroborated source.
...
- Every story has `locked_urls = [lead_url] + corroborated_by` exactly.
...
- YAML is valid (no tab indentation, balanced quotes inside string values, proper list indentation).
...
- No narrative — your output is YAML data only, no Markdown body.
